...
def check_ifc(
            ...
            for path_ids_file in normalized_ids_paths:
                ...
                try:
                    ...
                    report_metadata.append({
                        ...
                        "ifc_name": ifc_file_name,
                        "ids_name": ids_file_name,
                        "relative_path": os.path.relpath(html_file_path, folder_path_report),
                    })
                    
                    Parser_html.add_file_name_to_report(ifc_file_name, html_file_path)

                    success_message = f"[{counter}/{counter_sum}]{report_name}.html - готово"
                    ...
                    error_message = f"[{counter}/{counter_sum}]!Ошибка: {report_name}.html - {error}"
    ...
    if delete_empty_checks:
        ...
        _emit_status(status_callback, "Пропущенные проверки удалены из HTML-отчетов")
